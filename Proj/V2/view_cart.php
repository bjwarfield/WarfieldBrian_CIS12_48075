...
$q = 'SELECT `product_id` FROM `bladeshop`.`entity_products` AS `entity_products` WHERE `enabled` = true;';
...
$qp = 'SELECT `product_id`, `name`, `on_hand_qty`, `price` FROM `bladeshop`.`entity_products` AS `entity_products` WHERE `product_id` IN ('.substr(json_encode($cart_id_list), 1, -1).');';
